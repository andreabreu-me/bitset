...
class Bitset{
	public:
		...
		Bitset(const It& begin, const It& end) {
		...
		iterator operator [] (size_t idx) {
			return iterator(&value_[idx >> ( sizeof(value_type) + 2)], idx & (value_type_size- 1));
		}
		...
		bool test(size_t idx) const {
			...
			return value_[idx >> value_type_size] & (1 << (idx & value_type_mask));
		}
